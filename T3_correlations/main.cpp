...
///    global parameter values
    int resolution = 4;     // resolution of HEALPix map
    int enmax = 500;          // maximum and/or min (-enmax) of the set of integers constraining the wave-vectors of a fundamental domain
    int ellmax = 28;         // maximum ell of the spherical harmonic expansion of the temperature functions over pixel space
            ...
            for (int m = 0; m < eulAngData.size(); m++){
                ...
                ///    compute the eigenbasis for each domain and correlation matrices
                    ...
                    ///    (l,m) x (l',m') mode to mode correlations; where (l,m) = v (\nu)
                        Cvv = M.mode_mode(A, Rlss, eigB.k, eigB.basis, eigB.coeffs, transFuncData, eulAngData[m]);
