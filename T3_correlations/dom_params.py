...
lx_min, lx_max, lx_grid = 27, 35, 1
ly_min, ly_max, ly_grid = 27, 35, 1
lz_min, lz_max, lz_grid = 27, 35, 1
lengArr = []
for lx in range(lx_min, lx_max, lx_grid):
    for ly in range(ly_min, ly_max, ly_grid):
        for lz in range(lz_min, lz_max, lz_grid):
            domLeng = [lx, ly, lz]
            lengArr += [domLeng]
...
theta_min, theta_max, theta_grid = 0, 2*np.pi, 0.1*np.pi
phi_min, phi_max, phi_grid = 0, 2*np.pi, 0.1*np.pi
psi_min, psi_max, psi_grid = 0, 2*np.pi, 0.1*np.pi
angArr = []
...
alpha_min, alpha_max, alpha_grid = 0, 2*np.pi, 0.1*np.pi
beta_min, beta_max, beta_grid = 0, 2*np.pi, 0.1*np.pi
gamma_min, gamma_max, gamma_grid = 0, 2*np.pi, 0.1*np.pi
eulerArr = []
