...
//    std::cout << " stuff = " << std::sph_legendre(28, 28, 0.0006);
    if (m < 0){
        if (l < 0){ plm = pow(-1,m) * (factorial(abs(l-m)) / factorial(abs(l+m))) * std::assoc_legendre(abs(l-1), abs(m), x); }
        else { plm = pow(-1,m) * (factorial(abs(l-m)) / factorial(abs(l+m))) * std::assoc_legendre(l, abs(m), x); }
    }
    else {
        if (l < 0){ plm = std::assoc_legendre(abs(l-1), abs(m), x); }
        else { plm = std::assoc_legendre(l, m, x); }
    }
    if (x <= 0){
