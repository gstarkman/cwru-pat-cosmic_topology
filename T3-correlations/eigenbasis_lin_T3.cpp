...
std::vector<double> Kll(std::vector<int> nvec, std::vector<double> angles, std::vector<double> lengths){
    ...
    if (angles[0] == M_PI_2){
        ...
        k[2] = 2 * M_PI * (lengths[1]*lengths[2]*nvec[0]*std::cos(angles[1]) + lengths[0]*lengths[2]*nvec[1]*std::cos(angles[2]) - lengths[0]*lengths[1]*nvec[2]) / (lengths[0]*lengths[1]*lengths[2] * std::sqrt(std::sin(angles[1])*std::sin(angles[1]) - std::cos(angles[2])*std::cos(angles[2]) ));
    }
    else if (angles[1] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * ((nvec[1] / (std::sin(angles[0]) * lengths[1])) - (nvec[0] / (std::tan(angles[0]) * lengths[0])));
        k[2] = 2 * M_PI * ((lengths[2]*(lengths[1]*nvec[0]*std::cos(angles[0]) - lengths[0]*nvec[1])*std::cos(angles[2])*(1/std::sin(angles[1])) + lengths[0]*lengths[1]*nvec[2]*std::sin(angles[0])) / (lengths[0]*lengths[1]*lengths[2]*std::sqrt(std::sin(angles[2])*std::sin(angles[2]) - std::cos(angles[0])*std::cos(angles[0]))));
    }
    else if (angles[2] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * ((nvec[1] / (std::sin(angles[0]) * lengths[1])) - (nvec[0] / (std::tan(angles[0]) * lengths[0])));
        k[2] = 2 * M_PI * ((lengths[2]*(lengths[1]*nvec[0]*std::cos(angles[0]) - lengths[0]*nvec[1])*std::cos(angles[2])*(1/std::sin(angles[1])) + lengths[0]*lengths[1]*nvec[2]*std::sin(angles[0])) / (lengths[0]*lengths[1]*lengths[2]*std::sqrt(std::sin(angles[1])*std::sin(angles[1]) - std::cos(angles[0])*std::cos(angles[0]))));
    }
    else if (angles[0] == M_PI_2 && angles[1] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * nvec[1] / lengths[1];
        k[2] = 2 * M_PI * ((lengths[1] * nvec[2] - lengths[2] * nvec[1] * std::cos(angles[2])) / (lengths[1] * lengths[2] * std::sin(angles[2])));
    }
    else if (angles[1] == M_PI_2 && angles[2] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * ((nvec[1] / (std::sin(angles[0]) * lengths[1])) - (nvec[0] / (std::tan(angles[0]) * lengths[0])));
        k[2] = 2 * M_PI * nvec[2] * std::sin(angles[0]) / lengths[2];
    }
    else if (angles[2] == M_PI_2 && angles[0] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * nvec[1] / lengths[1];
        k[2] = 2 * M_PI * (lengths[0] * nvec[2] - lengths[2] * nvec[0] * std::cos(angles[1])) / (lengths[0] * lengths[2] * std::sin(angles[1]));
    }
    else if (angles[1] == M_PI_2 && angles[2] == M_PI_2 && angles[0] == M_PI_2){
        k[0] = 2 * M_PI * nvec[0] / lengths[0];
        k[1] = 2 * M_PI * nvec[1] / lengths[1];
    ...
    if (k[0] < .000000000000001 && k[0] > 0.0) k[0] = 0.0;
    if (k[1] < .000000000000001 && k[1] > 0.0) k[1] = 0.0;
    if (k[2] < .000000000000001 && k[2] > 0.0) k[2] = 0.0;
    return k;
}
...
std::vector<double> Khx(std::vector<int> nvec, std::vector<double> angles, std::vector<double> lengths){
    ...
    if (angles[1] == M_PI_2){
        ...
        k[0] = 2 * M_PI * nvec[0] / L1;
        k[1] = 2 * M_PI * nvec[1] / L2;
...
std::complex<double> NConst (std::vector<double> lengths, std::vector<double> kvec, std::vector< std::complex<double> > coeffs, std::vector< std::vector< std::vector<double> > > transMats){
    ...
    for (int q = 0; q < q_max; ++q){
        for (int qp = 0; qp < q_max; ++qp){
            if (q != qp){
                for (int p = 0; p < 3; ++p){
                    denom *= 1 / (v[q][p] - v[qp][p]);     /// compute 1 / /prod_p(v_{qp} - v_{q'p})
    ...
    if (domIndx == 1){
        ...
        for (int n = 0; n < nsz; n++){
            ...
            for (int i = 0; i < Cf.size(); i++){
                ...
                for (int m = 0; m < Cf.size(); m++){ //perform sum over q and q'
                    esum += Cf[i]*xiterm[i] * std::conj(Cf[m]*xiterm[m]);
                }
            }//i
