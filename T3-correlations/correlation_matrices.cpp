...
std::vector< std::vector< std::complex<double> > > correlation_matrices::mode_mode(double amp, double R_LSS, std::vector< std::vector< double > > k, std::vector< std::complex< double > > basis, std::vector< std::vector< double > > interpIn, std::vector< double > eulAngs){
...
///    correlation matrix
    ...
    std::complex<double> nSumTerm = 0.0; // sum of terms that only depend on nvec
    ...
    ///    matrix computation
        if (knorm == 0){
            nSumTerm += std::complex<double>(0.0, 0.0) * basis[n];
        }
        else {
            nSumTerm += (1 / (knorm*knorm*knorm) ) * basis[n];
        }
        for (int v = 0; v < corrsize; v++){
            for (int pv = 0; pv < corrsize; pv++){
                corrmatVal =  std::pow(imu, (lvec[v]-lvec[pv])) *
                ...
                modeMat[v][pv] *= 16 * M_PI * M_PI * amp * nSumTerm;
            }//pv
        }//v
    }//n
    return modeMat;
